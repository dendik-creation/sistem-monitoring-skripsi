...
@section('content')
    <div class="container-fluid mt-4">
        ...
        {{-- [Syahrul][05/05/2025] Penambahn pesan dari error download --}}
        @if (session('info'))
        ...
        <div class="row mt-5">
            @foreach ($data as $item)
                    ...
                    <table class="table table-borderless">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>:</td>
                                <th><a
                                        href="/download/{{ $item->nim }}/proposal/{{ $item->proposal }}"><?= $item->proposal == null ? '' : 'Download file proposal mahasiswa' ?></a>
                                </th>
                            </tr>
                            <tr>
                                <td>Keterangan</td>
                    ...
                    <div class="ml-2 mt-4">
                        @if ($item->ket1 == 'Menunggu ACC' || $item->ket2 == 'Menunggu ACC')
                            <button type="button" class="btn btn-primary mr-2" data-toggle="modal"
                                data-target="#modalrev{{ $item->id }}" disabled>
                ...
                <div class="modal fade" id="modalrev{{ $item->id }}" role="dialog" aria-labelledby="exampleModalLabel"
                    aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <form action="/mahasiswa/proposal/revisi/{{ $item->id }}" method="post"
                                enctype="multipart/form-data">
                                {{ csrf_field() }}
                                {{ method_field('PUT') }}
                                <div class="modal-body">
                                    <div class="form-group">
                                        <label for="" class="small">Revisi Proposal* (DOCX, DOC, PDF) (Max
                                            10MB)</label><br>
                                        <input type="file" name="proposal" placeholder="Masukkan File"
                                            accept=".doc, .docx, .pdf">
                                    </div>
                                    <div class="form-group">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <th>
                                    ...
                                    <p style="pointer-events: none;"
                                        class="btn btn-sm mt-2 <?= $item->ket1 == 'Disetujui' ? 'btn-success' : ($item->ket1 == 'Revisi' ? 'btn-warning' : ($item->ket1 == 'Ditolak' ? 'btn-danger' : 'btn-secondary')) ?>">
                                        {{ $item->ket1 }}
                                </th>
                            </tr>
                            <tr>
                                ...
                                <th>
                                    ...
                                    @else
                                        @if ($dosen2->depan == 'Y')
                                            ...
                                            {{ $dosen2->gelar2 }} - <p style="pointer-events: none;"
                                                class="btn btn-sm mt-2 <?= $item->ket2 == 'Disetujui' ? 'btn-success' : ($item->ket2 == 'Revisi' ? 'btn-warning' : ($item->ket2 == 'Ditolak' ? 'btn-danger' : 'btn-secondary')) ?>">
                                                {{ $item->ket2 }}
                                            @else
                            ...
                            <tr>
                                ...
                                <th>
                                    ...
                                    @else
                                        {{ $item->komentar1 }} - <a
                                            href="/download/{{ $item->nim }}/revisiproposal/{{ $item->file1 }}">Download
                                            file revisi proposal</a>
                                </th>
            @endif
            </th>
            </tr>
            <tr>
                ...
                <th>
                    ...
                    @else
                        ...
                        @else
                            {{ $item->komentar2 }} - <a
                                href="/download/{{ $item->nim }}/revisiproposal/{{ $item->file2 }}">Download file
                                revisi proposal</a>
                </th>
                @endif
                @endif
