            ...
            <div class="row mt-2">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="smt" placeholder="Masukkan Semester"
                            value="{{ $data->smt }}" required readonly>
                    </div>
                    {{-- <div class="form-group">
                            ...
                            @foreach ($dosen1 as $item)
                                @if ($item->depan == 'Y')
                                    <option value="{{ $item->nidn }}"
                                        <?= $data->dosbing1 == $item->nidn ? 'selected' : '' ?>>{{ $item->gelar3 }}
                                        {{ $item->name }}, {{ $item->gelar1 }}, {{ $item->gelar2 }}</option>
                                @else
                                    @if ($item->depan == null)
                                        <option value="{{ $item->nidn }}"
                                            <?= $data->dosbing1 == $item->nidn ? 'selected' : '' ?>>{{ $item->name }},
                                            {{ $item->gelar1 }}, {{ $item->gelar2 }}</option>
                                    @else
                                        <option value="{{ $item->nidn }}"
                                            <?= $data->dosbing1 == $item->nidn ? 'selected' : '' ?>>{{ $item->name }},
                                            {{ $item->gelar1 }}, {{ $item->gelar2 }}, {{ $item->gelar3 }} </option>
                                    @endif
                                @endif
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="dosbing2">
                            ...
                            @foreach ($dosen2 as $item)
                                @if ($item->depan == 'Y')
                                    <option value="{{ $item->nidn }}"
                                        <?= $data->dosbing2 == $item->nidn ? 'selected' : '' ?>>{{ $item->gelar3 }}
                                        {{ $item->name }}, {{ $item->gelar1 }}, {{ $item->gelar2 }}</option>
                                @else
                                    @if ($item->depan == null)
                                        <option value="{{ $item->nidn }}"
                                            <?= $data->dosbing2 == $item->nidn ? 'selected' : '' ?>>{{ $item->name }},
                                            {{ $item->gelar1 }}, {{ $item->gelar2 }} </option>
                                    @else
                                        <option value="{{ $item->nidn }}"
                                            <?= $data->dosbing2 == $item->nidn ? 'selected' : '' ?>>{{ $item->name }},
