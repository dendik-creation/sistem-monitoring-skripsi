        ...
        <form class="user" action="/admin/proposal/plotting/insert" method="POST">
            ...
            <div class="row mt-2">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="smt" placeholder="Masukkan Semester"
                            value="{{ $smt->semester }} {{ $smt->tahun }}" required readonly>
                    </div>
                    {{-- <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="" class="small">Mahasiswa*</label>
                        <select class="form-control" name="nim">
                            <option value="" readonly>Mahasiswa --</option>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="dosbing1">
                            ...
                            @foreach ($dosen1 as $item)
                                @if ($item->depan == 'Y')
                                    <option value="{{ $item->nidn }}">{{ $item->gelar3 }} {{ $item->name }},
                                        {{ $item->gelar1 }}, {{ $item->gelar2 }}</option>
                                @else
                                    @if ($item->depan == null)
                                        <option value="{{ $item->nidn }}">{{ $item->name }}, {{ $item->gelar1 }},
                                            {{ $item->gelar2 }}</option>
                                    @else
                                        <option value="{{ $item->nidn }}">{{ $item->name }}, {{ $item->gelar1 }},
                                            {{ $item->gelar2 }}, {{ $item->gelar3 }} </option>
                                    @endif
                                @endif
                            @endforeach
                        </select>
